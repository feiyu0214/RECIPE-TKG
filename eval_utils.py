from transformers import AutoModelForCausalLM, BitsAndBytesConfig, AutoConfig, LlamaForCausalLM
...
def read_last_metric(last_metric, apply_semantic_filter):
    if last_metric != '': 
...
def parse_args():
    ...
    parser.add_argument('--instruct_yes', type=int, default=0, help='Set 0 to give no instruction in the prompts. ')
    parser.add_argument('--apply_semantic_filter', default=False, action="store_true", help='Whether to apply semantic filtering during inference.')
    parser.add_argument('--similarity_threshold', type=float, default=0.6, help='threshold')
